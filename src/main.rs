...
async fn main() -> std::io::Result<()> {
    ...
    let service_config = ServiceConfiguration::load_from_file("config.toml")
        .map_err(|e| Error::other(format!("{:?}", e)))?;
    
    let broadcast_actor = Arc::new(actix::Actor::start(BroadcastActor{}));
    for service in service_config.services {
        let actor = HttpHealthCheckActor::new(
            service.name,
            service.url,
            service.timeout_seconds.map_or(service_config.global.timeout_seconds, |v| v),
